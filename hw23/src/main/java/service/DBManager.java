...
public class DBManager {
    ...
    private static final String SELECT_USER_FROM_NAME = "select * from user where name=?";

    public static User finduser(String name) {
        ResultSet resultSet = null;
        User user = null;
        resultSet = executeQueryToParametr(SELECT_USER_FROM_NAME, name);
        if (resultSet != null) {
            try {
                while (resultSet.next()) {
                    ...
                    user = new User(idUser, nameuser, password, firstname);
                }
